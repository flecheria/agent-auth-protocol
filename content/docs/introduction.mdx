...
Agent Auth is an open protocol that gives AI agents their own identity. Instead of inheriting the identity of the application or the user, each agent gets its own cryptographic keypair, its own granted capabilities, and its own lifecycle.
...
When an agent acts on behalf of a user, it typically inherits the identity of the application or the user — operating under existing credentials with no way to identify the agent as a separate principal. That collapses the agent into someone else's identity:
